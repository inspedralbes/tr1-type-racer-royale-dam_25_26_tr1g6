...
const hasWorkspace = index.hasFile("pnpm-workspace.yaml") || index.hasFile("pnpm-workspace.yml");
function pnpm() {
  return [
    {
...
async function buildConfig(maybeOptions, ...userConfigs) {
  ...
  const pnpmEnabled = vOptions.pnpm ?? (await index.getPackageManager())?.name === "pnpm";
  if (pnpmEnabled) {
