<template>
  <v-app>
    <v-app-bar color="red-darken-3" dark flat>
      <v-toolbar-title>Buscador de Ejercicios PRO (con Gemini)</v-toolbar-title>
    </v-app-bar>
        ...
        <v-dialog v-model="detailsDialog" max-width="700">
          <v-card v-if="selectedExercise">
            ...
            <v-card-text class="pt-4">
              ...
              <strong>Instrucciones (por Gemini PRO):</strong>
              <div v-if="instructionsLoading" class="text-center pa-4">
                <v-progress-circular indeterminate color="red-darken-3"></v-progress-circular>
                <p class="mt-2">Generando instrucciones...</p>
              </div>
              <div v-else class="mt-2" style="white-space: pre-wrap;">
                {{ generatedInstructions }}
              </div>
              
              <v-divider class="my-4"></v-divider>
              <strong>Demostración Visual:</strong>
              <v-img v-if="selectedExercise.gifUrl" :src="selectedExercise.gifUrl.replace('http://', 'https://')" class="mt-2 border"></v-img>
              <p v-else class="mt-2">No hay GIF disponible para este ejercicio.</p>
            </v-card-text>
...
async function showDetails(exercise) {
  ...
  try {
    ...
    Genera instrucciones claras, detalladas y concisas, paso a paso, para realizar el siguiente ejercicio en español.
    
    Ejercicio: "${exercise.name}"
    Músculo Principal: ${translateToSpanish(exercise.target)}
    Equipamiento: ${translateToSpanish(exercise.equipment)}

    Formatea la respuesta de manera legible, separando los pasos (por ejemplo, con guiones o números). Incluye una breve sección de 'Preparación' y 'Ejecución'.
    `;
    ...
    generatedInstructions.value = 'Error: No se pudieron generar las instrucciones. Inténtalo más tarde.';
  } finally {
