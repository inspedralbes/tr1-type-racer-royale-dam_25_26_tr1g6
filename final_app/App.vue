...
const { ws, wsConnected, send } = useWebSocket(BACKEND_URL, {
  leaderboard_update: (data) => {
    leaderboard.value = data.payload.leaderboard
  },
  session_info: (data) => {
...
function handleStartTrainingFromExercises(data) {
  // selectedExerciseId.value = data.exercise.id // Ya no necesitamos esto
  userNameCreate.value = data.userName
  
  // Pasamos el nombre del ejercicio directamente a createSession
  createSession(data.exercise.name)
}

// // Create a new training session
// Ahora acepta 'exerciseName' como argumento
function createSession(exerciseName) {
  ...
  // Ya no buscamos el ejercicio, usamos el nombre que nos pasaron
  ...
  send('join', {
    ...
    payload: {
      ...
      exercise: currentExerciseName.value // ¡Este nombre ahora será correcto!
    }
  })
