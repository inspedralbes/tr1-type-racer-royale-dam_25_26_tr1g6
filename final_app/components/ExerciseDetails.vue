      ...
      <v-container class="py-6 py-md-10">
        ...
        <v-row v-if="!(loading && (loadingType === 'initial' || loadingType === 'search'))">
          <v-col v-for="exercise in filteredExercises" :key="exercise.id" cols="12" sm="6" md="4" lg="3">
            ...
            <v-card 
              class="exercise-card mx-auto my-3 d-flex flex-column" 
              max-width="400" 
              elevation="4" 
              rounded="lg"
              style="height: 100%;"
            >
              ...
              <div v-intersect="(isIntersecting) => handleIntersection(isIntersecting, exercise)"></div>
            </v-card>
          </v-col>
        ...
        <v-dialog v-model="showSelectionDialog" max-width="500px">
          <v-card class="bg-surface rounded-xl">
            ...
            <v-card-text class="py-6">
              <p class="mb-4">
                You selected: <strong class="text-secondary">{{ exerciseToTrain?.name }}</strong>
              </p>
              <v-text-field
...
<script setup>
// Imports del Componente 1
import { ref, computed, onMounted, onUnmounted, nextTick } from 'vue';
import { GoogleGenerativeAI } from "@google/generative-ai";
import { useTheme } from 'vuetify';
// AÑADIDO: defineEmits (del Componente 2)
import { defineEmits } from 'vue';

// EMITS AÑADIDOS (del Componente 2)
...
// --- ESTADO AÑADIDO (del Componente 2) ---
const showSelectionDialog = ref(false);
// Renombrado para evitar conflicto con selectedExercise
const exerciseToTrain = ref(null); 
const trainingUserName = ref('');

// --- APIs y Lógica del Componente 1 ---
...
// Computed del Componente 1
// (Nota: El filtro de Dificultad del Comp 2 no se puede añadir aquí
// porque la API del Comp 1 no devuelve ese dato)
...
// --- FUNCIONES AÑADIDAS (del Componente 2) ---
...
function selectExercise(exercise) {
  // Usamos la variable de estado renombrada
...
// --- Resto de Funciones del Componente 1 ---
...
onMounted(() => {
  fetchInitialExercises();
  // (Nota: la función loadExercises() del Comp 2 no se usa
  // porque estamos usando la API del Comp 1)
...
/* Estilos AÑADIDOS del Componente 2 
*/
.gap-2 {
  gap: 0.5rem;
}
