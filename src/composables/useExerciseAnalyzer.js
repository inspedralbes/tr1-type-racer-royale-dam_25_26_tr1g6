...
export function useExerciseAnalyzer(exerciseName, sendRepetition) {
  ...
  // Bicep Curl analysis (track elbow angle)
  function analyzeBicepCurl(pose) {
    const rs = getKeypoint(pose, 'right_shoulder')
    const re = getKeypoint(pose, 'right_elbow')
    const rw = getKeypoint(pose, 'right_wrist')
    ...
    currentQuality.value = Math.max(0, 1 - Math.abs(angle - 50) / 50)
    ...
    // Up position: arm flexed (< 60 degrees)
    else if (isInDownPosition && angle < 60) {
      const now = Date.now()
      if (now - lastRepTime > REP_COOLDOWN) {
        sendRep(0.95, 'Nice curl! 💪')
      }
    } else {
      repState.value = 'neutral'
    }
  ...
  function analyzeShoulderPress(pose) {
    const rs = getKeypoint(pose, 'right_shoulder')
    const re = getKeypoint(pose, 'right_elbow')
    const rw = getKeypoint(pose, 'right_wrist')
    ...
    const angle = calculateAngle(rs, re, rw)
    currentAngle.value = angle
    currentQuality.value = Math.max(0, 1 - Math.abs(angle - 170) / 40)

    // Down position: arms bent (elbow < 90)
    if (angle < 90) {
      repState.value = 'down'
      isInDownPosition = true
      feedback.value = 'Push up!'
    }
    // Up position: arms extended (angle > 160)
    else if (isInDownPosition && angle > 160) {
      const now = Date.now()
      if (now - lastRepTime > REP_COOLDOWN) {
        sendRep(0.9, 'Great press! 🔥')
      }
    } else {
      repState.value = 'neutral'
    }
  ...
  // Sit-up analysis (torso angle)
  function analyzeSitUp(pose) {
    const sh = getKeypoint(pose, 'left_shoulder')
    ...
    const angle = calculateAngle(sh, hip, knee)
    currentAngle.value = angle
    currentQuality.value = Math.max(0, 1 - Math.abs(angle - 70) / 40)

    // Up position: torso raised (angle < 100)
    if (angle < 100) {
      isInDownPosition = true
      repState.value = 'up'
      feedback.value = 'Nice sit-up! Hold for a sec...'
    }
    // Down position: torso flat again (>150)
    else if (isInDownPosition && angle > 150) {
      const now = Date.now()
      if (now - lastRepTime > REP_COOLDOWN) {
        sendRep(0.95, 'Rep completed! 🎉')
      }
  ...
  function analyzeExercise(pose) {
    ...
    if (exercise === 'squat') analyzeSquat(pose)
    else if (exercise === 'push-up' || exercise === 'pushup') analyzePushUp(pose)
    else if (exercise === 'lunge') analyzeLunge(pose)
    else if (exercise === 'jumping jack' || exercise === 'jumpingjack') analyzeJumpingJack(pose)
    else if (exercise === 'plank') analyzePlank(pose)
    else if (exercise === 'bicep curl' || exercise === 'curl') analyzeBicepCurl(pose)
    else if (exercise === 'shoulder press' || exercise === 'press') analyzeShoulderPress(pose)
    else if (exercise === 'sit-up' || exercise === 'situp') analyzeSitUp(pose)
  }
